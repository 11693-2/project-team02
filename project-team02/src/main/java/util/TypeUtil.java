...
public class TypeUtil {
	...
	public static final Comparator<SearchResult> SEARCH_RESULT_SCORE_COMPARATOR = Comparator.comparing(
			SearchResult::getScore).reversed();

	public static <T extends SearchResult> List<T> rankedSearchResultsByScore(Collection<T> results, int hitSize) {
		List<T> sorted = results.stream().sorted(SEARCH_RESULT_SCORE_COMPARATOR).limit(hitSize).collect(toList());
	...
	private static <T extends SearchResult> Collection<T> rankedSearchResultsByRank(Collection<T> results) {
		return results.stream().sorted(SEARCH_RESULT_RANK_COMPARATOR).collect(toCollection(ArrayList::new));
	}
